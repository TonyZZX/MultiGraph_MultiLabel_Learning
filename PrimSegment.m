function [ graphs ] = PrimSegment( image, nodeNum, heapNum )
% [L, N] = superpixels(image, num)
% L: segment result
% N: the number of segments you get
% nodeNum: the number of segments you want
[L, N] = superpixels(image, nodeNum);
rowNum = size(image, 1);
colNum = size(image, 2);
...
graphs = Prim(centralCoordinates, std_nodes, std_edges, heapNum);
